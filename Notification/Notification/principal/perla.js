const claveRegistros = 'registrosArchivo1'; 
...
function exportarExcel() {
   ...
   XLSX.writeFile(wb, "registroscalidad.xlsx");
}

function verificarFechas() {
   const registros = document.querySelectorAll('#tablaRegistros tbody tr');
   const fechaHoy = new Date();
   registros.forEach(fila => {
       const celdas = fila.getElementsByTagName('td');
       const fechaVencimiento = new Date(celdas[5].textContent); 

       const diferenciaDias = Math.floor((fechaVencimiento - fechaHoy) / (1000 * 60 * 60 * 24));

       if (diferenciaDias <= 7 && diferenciaDias > 0) {

           if (Notification.permission === "granted") {
               new Notification("¡Alerta de vencimiento!", {
                   body: `La descripción '${celdas[2].textContent}' está por vencer en ${diferenciaDias} días.`,
                   icon: 'imagen/1.jpeg', 
               });
           }
       } else if (diferenciaDias <= 30 && diferenciaDias > 7) {
           // Notificación para 1 mes antes
           if (Notification.permission === "granted") {
               new Notification("¡Alerta de vencimiento en un mes!", {
                   body: `La descripción '${celdas[2].textContent}' vencerá en un mes.`,
                   icon: 'url_de_tu_icono.png',
               });
           }
       }
   });
...
const transporter = nodemailer.createTransport({
   service: 'gmail',
   auth: {
       user: 'dkfkfkxkkxx7@gmail.com',
       pass: 'Anderson2811', 
   },
});

function enviarCorreo(descripcion, fechaVencimiento) {
   const mailOptions = {
       from: 'dkfkfkxkkxx7@gmail.com',
       to: 'angiecubilla8@gmail.com',  
       subject: 'Recordatorio de vencimiento',
       text: `La descripción '${descripcion}' está por vencer el ${fechaVencimiento}.`,
   };
...
function verificarFechasYEnviarCorreo() {
   const registros = document.querySelectorAll('#tablaRegistros tbody tr');
   const fechaHoy = new Date();
   registros.forEach(fila => {
       const celdas = fila.getElementsByTagName('td');
       ...
       if (diferenciaDias <= 7 && diferenciaDias > 0) {
           enviarCorreo(celdas[2].textContent, fechaVencimiento.toLocaleDateString());
       }
   });
}